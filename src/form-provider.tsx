...
export interface FormProviderProps<T extends {}> {
  /**
   ...
   */
  onChange?: (context: ReturnType<FormProvider<T>["getContextValue"]>, attribute: string) => void;
  /**
   *
...
export class FormProvider<T extends Record<string, any>> extends React.Component<
  FormProviderProps<T>,
  FormProviderState<T>
> {
  /**
  ...
  setAttribute = (attribute: string, value: any) => {
    ...
    const timeout = this.props?.validateTimeout || 800;
    if (timeout > 0) {
      this.queueValidation(attribute, timeout);
    }
